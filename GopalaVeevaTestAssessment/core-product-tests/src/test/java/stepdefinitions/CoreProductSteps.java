...
public class CoreProductSteps {
	...
	public void navigate_to_men_s_page() {
		...
		// Close banners if any
		if (util.isElementVisble(mensPage.closeFreeShipping)) {
	...
	public void identify_and_count_the_number_of_video_feeds() {
		...
		/*
		 * for(WebElement vid : videoFeedElements) { System.out.println("hello........."
		 * + vid.getClass()); }
		 */
		...
		/*
		 * List<WebElement> videoFeedElements =
		 * util.findElements(newsAndFeatures.timeValuesOfVideos); int totalfeeds = 0;
		 * int morethan3days = 0; String videoDuration=""; for (WebElement video :
		 * videoFeedElements) { // Retrieve video duration totalfeeds += 1;
		 * videoDuration = video.getText(); if (videoDuration.endsWith("d")) {
		 * System.out.println(videoDuration); int days =
		 * Integer.parseInt(videoDuration.replace("d", "")); if (days > 3) {
		 * morethan3days += 1; } } }
		 */
	...
	public void find_the_count_of_videos_which_are_older_than_3_days() {
		...
		for (int days : videoDurations) {
			if (days >= 3) {
				count++;
				// System.out.println("check2 ....." + videoDurations);
			}
